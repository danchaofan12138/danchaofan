...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.tkzxbd : $.getdata("tkzxbd")) || "";
let msg, ck;
//---------------------------------------------------------------------------------------------------------
...
!(async () => {
	if (typeof $request !== "undefined") {
		...
		let ckArr = await Variable_Check(ckStr, "tkzxbd");
		await tips(ckArr);
...
/**
 * 签到    hthttpPost
 *https://m.tk.cn/activity_execute/rest/membergoldbean/sign
 */
   ...
   $.post(url,async(error, response, data)  => {
            try {
                ...
                if (result.error_code == 0) {
                    $.log(`\n【🎉🎉🎉 恭喜你了 🎉🎉🎉】签到状态:(${result.error_message}) ,获得签到奖励  ✅ `)
                } else if (result.error_code == 200004200003) {
                    $.log(`\n【🎉 高兴个蛋 🎉】签到状态: ${result.error_message}明天再来`)
                } else {
                    $.log(`\n【🎉 高兴个蛋 🎉】执行签到:失败 ❌ 了呢 ${result.error_message}`)
                }
            } catch (e) {
                $.logErr(e, resp);
            } finally {
