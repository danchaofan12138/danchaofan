/*
小程序:锅圈商城 
只有签到功能，一个月可以兑换一份食材，到冬天应该可以免费吃一桌火锅。
登陆小程序之后，点击积分，进入到签到界面，点击签到获取ck，⚠️只有签到的时候获取到的ck才有用。
青龙自己抓包:https://apigw.zzgqsh.com/mapp/req/signInPoliteClient.signIn的请求头和body，用#连接，如:请求头#body，变量名:gqscck，支持多账号，多账号用@隔开
本地重写:
mapp/req/signInPoliteClient.signIn url script-request-body gqsc.js
hostname = apigw.zzgqsh.com
boxjs地址:
https://raw.githubusercontent.com/danchaofan12138/danchaofan/main/ziyong/danchaofan.json
脚本地址:
https://raw.githubusercontent.com/danchaofan12138/danchaofan/main/ziyong/guoquanshangcheng/gqsc.js
*/
...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.gqscck : $.getdata('gqscck')) || '';
let msg, ck, ck_status;
let host = 'mc.kukahome.com';
let hostname = 'https://' + host;
//---------------------------------------------------------------------------------------------------------
let VersionCheck = "0.1.0"
let Change = '增加圈x v2p、青龙兼容,自行测试吧!'
let thank = `\n感谢 蛋炒饭 的投稿\n`
//---------------------------------------------------------------------------------------------------------
...
!(async () => {
	if (typeof $request !== "undefined") {  // 严格不相等
		...
		let ckArr = await Variable_Check(ckStr, "gqscck");
		await tips(ckArr);
...
async function GetRewrite() {
	if ($request.url.indexOf("signInPoliteClient.signIn") > -1) {
		let ck_az = $request.body;
		let ck_id = JSON.stringify($request.headers);
		ck = `${ck_id}#${ck_az}`;
		if (ckStr) {
			if (ckStr.indexOf(ck_id) == -1) {  // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "gqscck");
				ckList = ckStr.split("@");
...
function qd( timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url: `https://apigw.zzgqsh.com/mapp/req/signInPoliteClient.signIn`,
            headers: JSON.parse(ck[0]),
            body: ck[1]
        }
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(ret.code == 200){
                    DoubleLog(`签到成功：已签到${ret.data.nextDays - 1}`);
