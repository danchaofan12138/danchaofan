...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.zajkqd_data : $.getdata("zajkqd_data")) || "";
let msg, ck;
let idnum = " ";
// let ck_status = true;
...
!(async () => {
	if (typeof $request !== "undefined") {
		...
		let ckArr = await Variable_Check(ckStr, "zajkqd_data");
		await tips(ckArr);
...
async function GetRewrite() {
	if ($request.url.indexOf("api/lemon/v1/common/activity/homePage") > -1) {
		const ck = $request.headers['Access-Token']
		if (ckStr) {
			if (ckStr.indexOf(ck) == -1) { // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "zajkqd_data");
				ckList = ckStr.split("@");
				$.msg($.name + ` 获取第${ckList.length}个 ck 成功: ${ck} ,不用请自行关闭重写!`);
			}
		} else {
			$.setdata(ck, "zajkqd_data");
 ...
 function zajkrwd() {
	return new Promise((resolve) => {
		...
		$.post(url, async (error, response, data) => {
			try {
				let result = JSON.parse(data);
   let idnum =`${JSON.stringify(result.result.valuableRewardList[0].awardDetailId)}`;
			resolve(idnum)
					console.log(idnum);
            }
              catch (e) {
...
async function zajklq(){
    let idnum = parseInt(await zajkrwd());
	await $.wait(2 * 1000);
     console.log(idnum);
	let url = {
		...
		body: `{"channelCode":"c20195660470001","activityCode":"ONA20220411001","id":${idnum}} `
